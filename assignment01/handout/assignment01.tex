...
\usepackage{xparse}
\ExplSyntaxOn
\cs_new_protected:Npn \IfBlankF #1#2 { \tl_if_blank:nF {#1} {#2} }
\ExplSyntaxOff
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\noindent\textit{回答：}错误。100 TFLOPS 只表示它能在 $1\,\mathrm{s}$ 完成 $10^{14}$ 次计算，可能来自大量 thread 的并行；每个 thread 的频率未必高于 5 GHz 的 CPU。因此若只执行单条指令，GPU 的延迟仍可能高于 CPU。
  \item HBM 的“高带宽”指大块连续访问时的吞吐，零散的随机访问达不到标称值。
  \par\noindent\textit{回答：}正确。HBM（High Bandwidth Memory）在连续、大块且并发足够的访问下，吞吐可能接近标称值；零散随机访问会因延迟、请求粒度和并发不足而使实际吞吐大幅下降。
  \item 严格串行的迭代算法（每步依赖上一步的结果），即使换一块算力更强的 GPU 也快不了多少。
  \par\noindent\textit{回答：}正确。如果算法严格串行，那就无法利用 GPU 的并发优势，只能一条指令一条指令的执行，瓶颈就在于单个指令的计算速度。而 GPU 算力的提升对频率影响不大，并不能快多少
  \item “算力 1000 TFLOPS”意味着每次运算的延迟是 $10^{-15}$ 秒。
  \par\noindent\textit{回答：}错误。TFLOPS 指 tera floating-point operations per second，指的是 1s 能执行 $10^{12}$ 次浮点计算，而不是指每次运算的延迟，算力的提升更多来自 thread 数的增多
\end{enumerate}

\prob{1.2}{CONCEPT}
Session 1 讲座里提过“N 方过百万”这个例子。总计算量 $10^{12}$ FLOP 在当代 GPU 上的运算时间大概是毫秒级，那为什么一个严格在线的串行算法仍然做不到几秒内跑完？（从“延迟”和“吞吐”的角度考虑）
\par\noindent\textit{回答：}首先 GPU 的吞吐是很大的，它可以同时进行大量计算，但是对于单条指令的执行，它并没有像 CPU 一样快。所以如果是严格串行，那么 $10^{12}$ FLOP 的计算量需要串行一条一条执行，相比 CPU 就会慢很多。GPU 是高吞吐，单条指令延迟较高。而串行算法会使 GPU 的吞吐降低，单条指令的延迟基本不变，所以串行算法要跑很久
...
\par\noindent\textit{回答：}SIMD 是 single instruction multiple data，SIMT 是 single instruction multiple threads。SIMD 中并行主要针对 data，一条指令下，同时处理多个 data 达到并行。而 SIMT 是针对的多个 thread。一条 kernel 下，多个 thread 可以并行执行 kernel 代码，每个 thread 各自拥有寄存器和数据 \\
...
\par\noindent\textit{回答：}(a)因为 GPU 的算力优势主要来自于大量的并行计算，而单个 thread 的频率远低于 CPU，所以导致 GPU 单线程极慢 \\
...
GPU \texttt{<<<1,1>>>} & 135.558ms & 32.32ns/elem\\
GPU \texttt{<<<1,256>>>} & 2.245ms & 0.54ns/elem\\
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\noindent\textit{回答：}\texttt{\_\_global\_\_}
  \item 只会被 kernel 调用的辅助函数。
  \par\noindent\textit{回答：}\texttt{\_\_device\_\_}
  \item host 和 device 代码都要调用的小工具函数。
  \par\noindent\textit{回答：}\texttt{\_\_host\_\_ \_\_device\_\_}
  \item 整个 kernel 运行期间不变、所有线程都要读的系数表。
  \par\noindent\textit{回答：}\texttt{\_\_constant\_\_}
  \item block 内线程共享的暂存数组。
  \par\noindent\textit{回答：}\texttt{\_\_shared\_\_}
\end{enumerate}
...
\par\noindent\textit{回答：}(a)因为 GPU 和 CPU 是异步的，必须等待 GPU 上的 kernel 执行完成后才能讲数据搬回 CPU。如果 GPU 上的 thread 还没执行完就读回，会发生 UB，所以必须要有一次同步，使 thread 全部执行完。原先版本中同步发生在调用 CUDA\_CHECK\_KERNEL() 中 \\
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\noindent\textit{回答：}错误。kernel 启动是异步的，当语句返回时，只表示 kernel 发射给了 GPU，但是不代表已经返回，还需要后续的 synchronize
  \item 同一个 stream 里，\texttt{cudaMemcpy}（device 到 host）会等它前面的 kernel 全部完成后才开始拷贝。
  \par\noindent\textit{回答：}正确。在同一个 stream 内操作按顺序执行，cudaMemcpy 会等 kernel 结束后再执行，且 CPU 本身对 cudaMemcpy 也是阻塞的
  \item kernel 内部的非法访存，会在启动语句处同步地报出来。
  \par\noindent\textit{回答：}错误。错误的报出是异步的，在 CPU 向 GPU 发射 kernel 之后，CPU 会继续执行，当 GPU 进行 synchronize 的时候，才会报出错误
\end{enumerate}
...
不加 CUDA\_CHECK\_KERNEL 时，由于 GPU 的执行与 CPU 是异步的，所以 CPU 并不会发现 GPU 上 kernel 的执行异常，所以不会报出错误
...
\par\noindent\textit{回答：}(a)顺序由 GPU 的调度决定，并没有严格的顺序，也就是说每个 block 的执行时机是不确定的\\
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\noindent\textit{回答：}线性编号是 43, 第 1 个 warp，第 11 个 lane
  \item 这个 block 一共占多少个 warp？
  \par\noindent\textit{回答：}一共占 2 个 warp
  \item 若 \texttt{blockDim = (33, 1, 1)}，占几个 warp？这样配置浪费在哪里？
  \par\noindent\textit{回答：}占 2 个 warp。这样的配置 浪费在第 1 个 warp 中只有 1 个 thread 可用，但是一整个 warp 都会被调度，寄存器也会被占用，产生浪费
\end{enumerate}
...
\par\noindent\textit{回答：}按 warp 划分 thread 的会更快，大致会快 2 倍\\
...
\par\noindent\textit{回答：}(a)因为读和写之间存在数据竞争。thread 先把数据放进 shared memory 中，需要等到 thread 全部写完才能正常倒序。但是如果不进行 sync 就直接写的话，由于 warp 的乱序执行，并不能保证需要反转的数据已经被 thread 写好，发生 UB，所以程序出错\\
...
\par\noindent\textit{回答：}kernel 分割，将大的 kernel 拆分成多个小的 kernel，让每个 kernel 开始于需要同步的地方，这样就能自动同步。或者直接使用 cooperative\_group 直接进行全 grid 的同步\\
...
\par\noindent\textit{回答：}二者的性能差距主要来自 warp 的 mask 机制。第一种会 mask 掉一半 的 thread，导致相同的计算量，前者会慢将近 2 倍。而后者每次执行计算的时候都是以整个 warp 为单位进行计算的，不存在 mask 问题，开销就会变小。还有一部分来自小于指令的开销小于计算取模\\
...
shared & 整个 block & block & 片上 & 程序员 \\
global & grid 中全部 thread & application & 片外 & 程序员 \\
constant & grid 中全部 thread & application & 片外 & 程序员 \\
L1 / L2 cache & / & 硬件管理 & 片上 & 硬件管理 \\
\bottomrule
\end{tabular}
\end{center}
...
\par\noindent\textit{回答：}因为系数仅有 8 个，占 32 字节，而 GPU 的最小缓存是 128 字节，所以一次缓存能把这 8 个系数全部放入 L2 Cache，GPU 在读取时相比 constant cache 的优势基本没有\\
如果需要访问的系数表很大，L2 Cache 需要反复更新时，由于读取 global memory 的速度远慢于读取 constant cache，此时会有较大的性能提升
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\noindent\textit{回答：}正确。local memory 属于每个 thread 私有的，但他放在 GPU 上，而不是 SM 中
  \item 对数组用运行期才知道的下标做索引，可能迫使它被放进 local memory。
  \par\noindent\textit{回答：}正确。通常情况下，如果用运行时的下标做索引，编译器就不知道要把什么存进寄存器，只能正常地处理数组然后读数，会放入 local memory。但是如果有一些激进优化，就会通过预测把数据存入寄存器，而不是 local memory
\end{enumerate}
...
\par\noindent\textit{回答：}我的私有化实现相比 naive 版，在 5090 上快了 114.96x，吞吐高了 115x。\\
提速主要来自吞吐的大幅提升。naive 版本中全 grid 的 thread 都在竞争数据，很容易导致 atomicAdd 串行，降低效率。而私有化版本先在每个 block 内部进行统计，统计结束后只需要根据统计好的结果再做一次 atomicAdd，大幅度减小了 data race 的比例，所以性能显著提高
...
\midrule
GB/s & 1891.2 & 1196.1 & 714.9 & 520.6 & 1621.0 & 860.1\\
\bottomrule
\end{tabular}
\end{center}

\par\noindent\textit{回答：}趋势是随着 stride 倍增，吞吐不断下降，当 stride 到 16 时，吞吐又上升到一个较高峰，然后吞吐再次下降\\
stride 小于 16 时下降，是因为 GPU 读取数据时，是以整个 warp 为单位读取的，stride 等于 1，一下能读取 128 字节，刚好放进一个缓存行，合并访存，效率最高。然后访存效率不断下降。同时由于读 + 写的内存大于 72 MB，而 72 MB 是实际 L2 Cache 的大小，所以 stride 为 8 时带宽很小。当 stride 为 16 时，读 4 MB 加上写 64 MB，刚好能放入 L2 Cache，所以速率有了大幅提高。但是 stride 为 32 时，又出现了一行只能存一个 float，带宽过小，于是再次下降
...
实测带宽 (GB/s) & 1572.4 & 1576.0 & 1575.5 & 1576.7 & 1591.7 & 817.8 \\
\bottomrule
\end{tabular}
\end{center}
...
\par\noindent\textit{回答：}(a)5090 的 shared memory 为 100 K，每个 SM 的最大常驻线程为 1536。而 block 中有 256 个 thread，所以理论上每个 SM 中最多存在 6 个 block，也就是 48 个 warp。以给 block 分配 18.0 KB 为例，此时用 100 除以 18，得到 5.55，向下取整，也就是每个 SM 上放 5 个 block，40 个 warp，occupancy 为 83.3\% \\
(b)因为 GPU 隐藏延迟需要有足够多的常驻 warp，这样当一个 warp 去执行延迟较高的访存等指令时，就可以由其他的常驻 warp 继续请求，或者进行计算，打满带宽。当 occupancy 下降时，SM 中的常驻 warp 下降，导致当一些 warp 请求时陷入等待，没有空闲的 warp 继续请求，导致带宽没有打满，效率下降\\
(c)100\% 到 50\% 带宽基本保持不变，当 occupancy 从 50\% 到 16.7\% 带宽发生大幅下降。这是因为 100 到 50 基本保持一个平台期，带宽基本达到理论上限，即使是 50 的 occupancy，在 SM 上常驻的 warp 也足以打满带宽，不需要多余的空闲 warp 进行补充。而 occupancy 下降到 16 之后，没有空闲的 warp 持续发起请求，而陷入漫长的等待，导致带宽大幅降低\\
...
\par\noindent\textit{回答：}(a)cudaEvent 计时可以当作 kernel 耗时\\
(b)host 计时不等 GPU 测的是 launch 一个 kernel 所需的时间，由于 kernel 的执行与 CPU 异步，所以 launch 以后就会自动返回，继续执行。host 计时等 GPu，测的是 launch 一个 kernel 加上 kernel 执行的时间\\

\prob{5.2}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item 同一个 stream 里的操作按提交顺序执行。
  \par\noindent\textit{回答：}正确。在同一个 stream 中按顺序执行，不同 stream 的执行顺序受 GPU 调度，但不保证
  \item kernel 启动后，host 代码立刻继续往下执行。
  \par\noindent\textit{回答：}正确。host 代码与 kernel 代码是异步的，当 kernel launch 后，继续往下执行 host 代码
  \item unified memory 下，CPU 访问一页正被 GPU 占用的内存，会触发缺页与页迁移。
  \par\noindent\textit{回答：}错误。这个依赖于硬件的 cudaDevAttrDirectManagedMemAccessFromHost 属性。如果硬件是软件一致性系统，那么 CPU 访问正在被 GPU 占用的内存，就会发生缺页；如果是硬件一致性系统，那 CPU 可以直接访问 GPU 上驻留的内存，不会发生迁移
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\noindent\textit{回答：}错误。tile 是显存里的一块不可变的区域，kernel 中对 tile 的改写都会产生一个新 tile，而不是原 tile 的改变
  \item 对 tile 的一次运算（如两个 tile 相加）由编译器映射到 block 内的多个线程上执行。
  \par\noindent\textit{回答：}正确。tile 是比 simt 更高的一种层次，表面上是两个 tile 相加，实际上由编译器内部决定哪些 thread 进行计算
  \item tile 模型与 SIMT 模型互斥，一个 CUDA 程序只能选一种。
  \par\noindent\textit{回答：}错误。tile 模型 与 SIMT 模型共存。SIMT 提供了更细的粒度，tile 提供了更高的层次
\end{enumerate}
...
\midrule
并行单位 & block 里的 thread & tile block & \\
编号 & \texttt{blockIdx}/\texttt{threadIdx} & \texttt{bid} & \\
数据分工 & 线程用全局下标来划分数据 & tile block 用 bid 来划分数据 & \\
边界处理 & \texttt{if} 判断 & 由编译器自动判断，先根据 TILE 大小划分，超出边界的由编译器处理，比如把超出边界的 mask 掉，防止异常访存 & \\
\bottomrule
\end{tabular}
\end{center}
...
\par\noindent\textit{回答：}(a)每个 tile 对应哪些元素块由 bid 决定，每个 tile 中的 thread 对应哪些元素由编译器决定\\
(b)边界条件判断，计算 thread 的全局索引，每个 thread 的分工\\
...
\par\noindent\textit{回答：}主要集中在 kernel 的参数上，因为主体代码基本没有任何变化，只是加了一个常数和 relu
...
\par\noindent\textit{回答：}行号，列号，grid 尺寸都还有对应，没有了边界保护，由编译器自动做边界保护
...
\begin{enumerate}[label=(\alph*)]
  ...
  \par\noindent\textit{回答：}错误。PTX 是中间表示，不是机器码，可直接执行的机器码是 SASS
  \item 只嵌入了 \texttt{sm\_70} SASS 的可执行文件，能在 compute capability 9.0 的卡上运行。
  \par\noindent\textit{回答：}错误。只能在 7.x 上跑
